...
// Run with: node test-db-balance-mapping.js
...
async function testDBBalanceMapping() {
  ...
  try {
    ...
    console.log("\n📋 Sample balance data:");
    balanceData.rows.slice(0, 5).forEach((row, index) => {
      console.log(
    ...
    for (const configData of testConfigs) {
      ...
      const testComptableConfig = await (prisma as any).DSFComptableConfig.create({
        data: {
